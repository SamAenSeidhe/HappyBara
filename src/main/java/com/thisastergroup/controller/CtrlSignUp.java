    ...
    public void SignUp() {
        ...
        try {
            ...
            User us = new User(txtfUsername.getText(), hashedPwd, txtfEmail.getText(),
                    cbGender.getValue(), (cbAge.getValue()), txtfCountry.getText(), 0,"", null);
            try {
                ...
                if (db_methods.userExists(us.getEmail()) || db_methods.usernameExist(us.getUsername())) {
                    ...
                    if (!us.checkPassword(passw)) {
                        ...
                        txtfPassword.clear();
                        mistake = false;
                        greetingMsg.setText("8 characters, 1 number, 1 uppercase, 1 lowercase, 1 (@#%$^), any space/tap");
                        greetingMsg.setStyle("-fx-font-size: 14px; -fx-font-family: 'Bookman Old Style';");
                        greetingMsg.setTextAlignment(TextAlignment.CENTER);
                    } else {
                        createMsg("");
                    }
