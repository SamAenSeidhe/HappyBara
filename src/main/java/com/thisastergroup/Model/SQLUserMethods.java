...
public class SQLUserMethods extends SQLConnection {
            ...
            if (rs.next()) {
                ...
                if (email.equals(rs.getString("email")) && BCrypt.checkpw(password, rs.getString("password"))) {
                    ...
                    return new User(rs.getString("username"), rs.getString("password"), rs.getString("email"),
                            ...
                            rs.getString("items"), rs.getString("outfit"));
    ...
    public void updateOutfit(User us) {
        try {
            String query = "UPDATE Users SET outfit = ?  WHERE username = ?";
            ps = conn.prepareStatement(query);
            ps.setString(1, us.getOutfit());
            ps.setString(2, us.getUsername());
            ps.executeUpdate();
        } catch (SQLException e) {
            System.err.println(e.getMessage());
    ...
    public String getOutfit(User us) {
        try {
            String query = "SELECT outfit FROM Users WHERE username = ?";
            ps = conn.prepareStatement(query);
            ps.setString(1, us.getUsername());
            rs = ps.executeQuery();
            if (rs.next()) {
                System.out.println("Getting outfit...");
                System.out.println("Outfit retrieved");

                return rs.getString("outfit");
            }
        } catch (SQLException e) {
            System.err.println(e.getMessage());
        } /*
    ...
    public void updateBalance(User us) {
        try {
            String query = "UPDATE Users SET balance = ?  WHERE username = ?";
            ps = conn.prepareStatement(query);
            ps.setInt(1, us.getBalance());
            ps.setString(2, us.getUsername());
            ps.executeUpdate();
        } catch (SQLException e) {
            System.err.println(e.getMessage());
