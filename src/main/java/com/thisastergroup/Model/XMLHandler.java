            ...
            for (int i = 0; i < CategoriesList.getLength(); i++) {
                ...
                if (CategoriesList.item(i).getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    if (categoryElement.getAttribute("name").equals(ActivityCategory)) {
                        ...
                        int randNum = rand.nextInt(category.getLength() - 1);
                        System.out.println("Lenght: " + category.getLength());
                        System.out.println("Number: " + randNum);

                        Question = null;
                        Tip = null;
                        while (Question == null || Tip == null) {
                            if (category.item(randNum).getNodeType() == Node.ELEMENT_NODE) {

                                // Lastly get the text and tip from the <question> node
                                NodeList a = category.item(randNum).getChildNodes();
                                ...
                                randNum = rand.nextInt(category.getLength() - 1);
                            }
                        }
